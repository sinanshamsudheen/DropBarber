...
export function AppointmentCard({ appointment }: { appointment: HydratedAppointment }) {
  return (
    <Link
      ...
      className="block rounded-2xl border border-border bg-card p-4 shadow-[var(--shadow-card)] transition-shadow hover:shadow-[var(--shadow-raised)]"
    >
      <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
        <div className="min-w-0">
          <h3 className="truncate text-base font-semibold">{appointment.shop.name}</h3>
